...
$$
\mathrm{MSE}(s) = \frac{1}{N}\sum_{i=1}^N
\left(x_i-s\cdot\operatorname{clip}\!\left(\mathrm{round}(x_i/s),-Q_n,Q_p\right)\right)^2
$$
...
这些值都没有被截断，因此每个值的绝对舍入误差不超过 $s/2\approx0.005$。小权重的相对误差可能更大；它对最终输出 $Wx$ 的影响还取决于对应输入 $x$ 的大小，不能只看权重本身判断是否无关紧要。
...
为了让实数 0 也能由整数 zero-point 表示，先取 $a=\min(x_{\min},0)$、$b=\max(x_{\max},0)$，再按 $[a,b]$ 计算参数（8 位无符号整数范围为 $[0,255]$）：

$$
s = \frac{b-a}{2^n-1}, \quad
z = \operatorname{clip}\!\left(\mathrm{round}\!\left(-\frac{a}{s}\right),0,2^n-1\right)
$$

直观推导：线性映射

$$
x_q \approx \frac{x-a}{b-a}\times(2^n-1)
$$

整理后得到 $s$、$z$ 的表达式。整数 $z$ 对应实数 0；但 $z$ 需要取整，因此区间端点通常只能近似映射。若 $a=b=0$，可约定正的 scale 并把所有值编码为 0；若张量是非零常数，则 $a\ne b$，仍可按上式量化，也可以在格式允许时单独存储常数。
...
但 OBQ 的逐参数贪心选择和二阶信息更新代价很高，直接扩展到 LLM 不现实。

### 7.2 GPTQ 的工程优化

GPTQ 保留二阶误差补偿的核心思路，同时改变选取顺序并优化计算：
...
AWQ 论文观察到：保护少量重要权重（论文以约 1% 为例）就能明显降低量化误差。判断哪些通道值得保护，要参考对应输入**激活值的大小**，不能只看权重本身。
